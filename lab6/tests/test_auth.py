...
USER = {"username": "user_test", "password": "user123"}
...
def test_register_returns_user_data(client: TestClient):
    data = client.post("/auth/register", json=USER).json()
...
def test_register_role_is_always_user(client: TestClient):
    data = client.post("/auth/register", json=USER).json()
    assert data["role"] == "user"
...
def test_register_username_too_short_returns_422(client: TestClient):
    assert client.post("/auth/register", json={**USER, "username": "ab"}).status_code == 422


def test_register_password_too_short_returns_422(client: TestClient):
    assert client.post("/auth/register", json={**USER, "password": "12345"}).status_code == 422
...
def test_login_returns_both_tokens(client: TestClient):
    client.post("/auth/register", json=USER)
    data = client.post("/auth/login", json={"username": USER["username"], "password": USER["password"]}).json()
...
def test_login_wrong_password_returns_401(client: TestClient):
    client.post("/auth/register", json=USER)
    assert client.post("/auth/login", json={"username": USER["username"], "password": "wrongpass"}).status_code == 401
...
def test_refresh_returns_200(client: TestClient):
    client.post("/auth/register", json=USER)
    tokens = client.post("/auth/login", json={"username": USER["username"], "password": USER["password"]}).json()
...
def test_refresh_returns_new_token_pair(client: TestClient):
    client.post("/auth/register", json=USER)
    t1 = client.post("/auth/login", json={"username": USER["username"], "password": USER["password"]}).json()
