...
def test_create_book_returns_correct_data(client: FlaskClient):
    data = client.post("/books/", json=TEST_BOOK).get_json()
...
def test_create_book_generates_unique_uuid(client: FlaskClient):
    r1 = client.post("/books/", json=TEST_BOOK).get_json()
    r2 = client.post("/books/", json=TEST_BOOK).get_json()
    assert len(r1["id"]) == 36
    assert r1["id"] != r2["id"]
...
def test_create_book_issued_status(client: FlaskClient):
    data = client.post("/books/", json={**TEST_BOOK, "status": "issued"}).get_json()
...
def test_combined_filter_and_sort(client: FlaskClient):
    client.post("/books/", json={**TEST_BOOK, "title": "Z", "author": "Tolkien", "year": 2000})
    client.post("/books/", json={**TEST_BOOK, "title": "A", "author": "Tolkien", "year": 1990})
    client.post("/books/", json={**TEST_BOOK, "title": "M", "author": "Rowling", "year": 1995})
...
def test_pagination_custom_limit(client: FlaskClient):
    for i in range(5):
        client.post("/books/", json={**TEST_BOOK, "title": f"Book {i}"})
...
def test_get_book_by_id_returns_200(client: FlaskClient):
    created = client.post("/books/", json=TEST_BOOK).get_json()
...
def test_delete_nonexistent_returns_404(client: FlaskClient):
    assert client.delete("/books/00000000-0000-0000-0000-000000000000").status_code == 404


def test_delete_twice_second_returns_404(client: FlaskClient):
    created = client.post("/books/", json=TEST_BOOK).get_json()
    client.delete(f"/books/{created['id']}")
...
def test_delete_does_not_affect_other_books(client: FlaskClient):
    b1 = client.post("/books/", json=TEST_BOOK).get_json()
    b2 = client.post("/books/", json={**TEST_BOOK, "title": "Book 2"}).get_json()
